...
async def test_regular_poll(api: TelegramAPI, chat_id: int | str) -> None:
    ...
    try:
        ...
        # Останавливаем опрос
        print(f"\n   Остановка опроса...")
        stopped = await api.stop_poll(chat_id, telegram_msg_id)
...
async def test_quiz(api: TelegramAPI, chat_id: int | str) -> None:
...
async def test_list_polls(api: TelegramAPI, chat_id: int | str) -> None:
    """Список опросов."""
    print("\n=== Список опросов ===")
    try:
        polls = await api.list_polls(chat_id=str(chat_id), limit=5)
...
async def main(base_url: str, chat_id: int | str, quiz_only: bool = False) -> None:
    async with TelegramAPI(base_url) as api:
        print(f"Тестирование опросов в чате {chat_id}")
        print(f"API: {base_url}")

        if not quiz_only:
            await test_regular_poll(api, chat_id)

        await test_quiz(api, chat_id)
        await test_list_polls(api, chat_id)
...
if __name__ == "__main__":
    ...
    asyncio.run(main(args.base_url, args.chat_id, args.quiz_only))
